...
void camera_converte(camera *cam, IplImage *image) {
  ...
  for(int y = 0; y < cam->altura / cam->escalaY; y++) {
    ...
    for(int x = (cam->largura - 1) / cam->escalaX; x >= 0; x--) {
      ...
      scaledY = y * cam->escalaY;
      scaledX = x * cam->escalaX;
      cam->quadro[scaledY][scaledX][2] = *pixel;
      pixel++;

      cam->quadro[scaledY][scaledX][1] = *pixel;
      pixel++;

      cam->quadro[scaledY][scaledX][0] = *pixel;
      pixel++;
    }
...
camera *camera_inicializa(int i) {
  ...
  if(capture) {
    ...
    if(image) {
      ...
      // calcula escala de redimensionamento da tela
      cam->escalaX = CAM_RES_W / ( image->width * 1.00 );
      cam->escalaY = CAM_RES_H / ( image->height * 1.00 );

      cam->capture = capture;
      cam->altura = image->height * cam->escalaY;
      cam->largura = image->width * cam->escalaX;
...
void camera_copia(camera *cam, unsigned char ***matriz, ALLEGRO_BITMAP *bitmap) {
  ...
  for(int y = 0; y < cam->altura; y++) {
      ...
      for(int x = 0; x < cam->largura; x++) { // Escala a imagem da câmera para o tamanho determinado
